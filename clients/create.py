...
for i in range(len(df.index)):
    if df.at[i,'Vorname'] == 1 or df.at[i,'Familienname'] == 1:
        print('finished')
        break
    vorname = df.at[i,'Vorname']
    familienname = df.at[i,'Familienname']
    shortname = df.at[i,'Kürzel']
    klasse = str(df.at[i,'Klasse'])
    code = str(df.at[i,'Nummer'])
    c = requests.get(url + 'create/' + vorname + '/' + familienname + '/' + shortname + '/' + klasse + '/' + code, headers={'Authorization': key})
